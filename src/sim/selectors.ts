...
import type { Building, BuildingType, PlayerState, ResourceStore, ResourceType } from "./types";
...
export function productionRate(building: Building): number {
  const definition = def(building.type);
  if (!definition.production || building.state !== "complete") return 0;
  return definition.production.baseRate + definition.production.perWorker * building.workerCount;
}
